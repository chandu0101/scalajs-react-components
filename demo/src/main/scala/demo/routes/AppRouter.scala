...
object AppRouter {
  ...
  val config = RouterConfigDsl[Page].buildConfig { dsl ⇒
    ...
    (trimSlashes
      ...
      | muiRoutes
      | reactPopoverRoutes
      | googleMapRoutes).notFound(redirectToPage(Home)(Redirect.Replace))
      .renderWith(layout)
  }
  ...
  val homePageMenu = Vector(
    ...
    HomePage.ComponentInfo(name = "Elemental UI", imagePath = g.elementaluiImage.toString, route = EuiPages(EuiRouteModule.Info), tags = Stream("materialui", "material", "framework")),
