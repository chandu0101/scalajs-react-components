...
object ReactPopOver {
  ...
  trait Style {
    def popover: TagMod =
      (Seq(
        ...
        boxShadowPreFixer("0 5px 10px rgba(0, 0, 0, 0.2)")).toTagMod
  ...
  class Backend($ : BackendScope[Props, State]) {
    ...
    def render(P: Props, S: State, C: PropsChildren): VdomElement = {
      <.div(
        ...
        ^.top := S.top.px,
        if (S.open) ^.left := S.left.px else EmptyVdom
      )(
        <.div(
  ...
  def apply(title: String = "", placement: String = "right", style: Style = new Style {})(
      children: VdomNode*) =
